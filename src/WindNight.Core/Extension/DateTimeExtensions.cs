...
namespace System
{
    ...
    public static class DateTimeExtensions
    {
        ...
        public static List<string> GeneratorDateStringList(this DateTime beginDate, DateTime? endDateParam = null, string format = "yyyyMMdd")
        {
            var endDate = DateTime.Now.Date;
            if (endDateParam.HasValue)
            {
            ...
            if (beginDate > endDate) return new List<string>();
            var list = new List<string>();
            for (var i = beginDate; i < endDate; i = i.AddDays(1))
            {
                list.Add(i.FormatDateTime(format));
            }
            return list;
        }
