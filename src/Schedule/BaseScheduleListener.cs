...
namespace Schedule
{
    public class BaseScheduleListener<T> : IScheduleListener where T : IJob
    {
        ...
        protected JobBaseInfo JobBaseInfo => new() { JobCode = JobCode, JobId = JobId, JobName = JobName };

        protected string JobInfo => $"{JobBaseInfo}";
        ...
        protected async Task DoNoticeAsync(string message, string extendInfo = "")
        {
            ...
            {
                await _scheduleNotice.DoNoticeAsync(JobBaseInfo,
                    $"{message}{(extendInfo.IsNullOrEmpty() ? "" : $"\n\n{extendInfo}")}");
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var isContinueRun = isOnceJob || depJobs.IsNullOrEmpty() ||
                                await WaitJobCompleted(origJobName, depJobs.Split(',').ToList(), HardInfo.Now.Date);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    var msg =
                        $"{JobInfo} begin time:{HardInfo.Now:yyyy-MM-dd HH:mm:sss}, autoClose:{autoClose}, runParams:{runParams}";
                ...
                {
                    JobBaseInfo.JobCode = jobCode;
                    await ctrl.StartJobSafety(JobBaseInfo, runParams, isOnceJob);
                }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (onceJob)
                {
                    ...
                    {
                        JobLogHelper.Error(
                            $"{JobInfo}:单次运行job结束后删除job失败，job ：{JobContext.CurrentJobBaseInfo}，返回结果：{delJobRet}", null,
                            nameof(JobWasExecuted));
                    }
                }
