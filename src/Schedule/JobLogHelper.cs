...
namespace Schedule
{
    public static class JobLogHelper
    {
        ...
        {
            LogHelper.Info($"{JobContext.CurrentJobBaseInfo}: {message}", url: FixActionName(actionName), millisecond: millisecond, traceId: JobContext.JobId);
        }

        public static void Warn(string message, Exception ex = null, string actionName = "", long millisecond = 0)
        {
            LogHelper.Warn($"{JobContext.CurrentJobBaseInfo}: {message}", ex, url: FixActionName(actionName), millisecond: millisecond, traceId: JobContext.JobId);
        }

        public static void Error(string message, Exception ex, string actionName = "", long millisecond = 0)
        {
            LogHelper.Error($"{JobContext.CurrentJobBaseInfo}: {message}", ex, url: FixActionName(actionName), millisecond: millisecond, traceId: JobContext.JobId);
        }
