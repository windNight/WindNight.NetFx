...
namespace Microsoft.AspNetCore.GRpc.HttpApi
{
    internal class HttpApiProviderServiceBinder<TService> : ServiceBinderBase where TService : class
    {
        ...
        private bool TryGetMethodDescriptor(string methodName,
        ...
        private (TDelegate invoker, List<object> metadata) CreateModelCore<TDelegate>(string methodName,
            Type[] methodParameters, string verb, HttpRule httpRule, MethodDescriptor methodDescriptor)
        ...
        private MethodInfo? GetMethod(string methodName, Type[] methodParameters)
        {
            ...
            while (currentType != null)
            {
                ...
                if (matchingMethod == null) return null;
        ...
        private static class Log
        {
            private static readonly Action<ILogger, string, Type, Exception?> _streamingMethodNotSupported =
                LoggerMessage.Define<string, Type>(LogLevel.Warning, new EventId(1, "StreamingMethodNotSupported"),
                    "Unable to bind {MethodName} on {ServiceType} to HTTP API. Streaming methods are not supported.");

            private static readonly Action<ILogger, string, Type, Exception?> _methodDescriptorNotFound =
                LoggerMessage.Define<string, Type>(LogLevel.Warning, new EventId(2, "MethodDescriptorNotFound"),
