...
// Licensed under the Apache License, Version 2.0. See License.txt in the proje.cs root for license information.
...
namespace GRpc.Shared.HttpApi
{
    internal static class ServiceDescriptorHelpers
    {
        public static ServiceDescriptor? GetServiceDescriptor(Type serviceReflectionType)
        {
            ...
            if (property != null) return (ServiceDescriptor?) property.GetValue(null);

            throw new InvalidOperationException($"Get not find Descriptor property on {serviceReflectionType.Name}.");
        }

        public static bool TryResolveDescriptors(MessageDescriptor messageDescriptor, string variable,
            [NotNullWhen(true)] out List<FieldDescriptor>? fieldDescriptors)
        {
            ...
            while (path.Length > 0)
            {
                ...
                if (fieldDescriptors == null) fieldDescriptors = new List<FieldDescriptor>();
        ...
        private static object ConvertValue(object value, FieldDescriptor descriptor)
        {
            switch (descriptor.FieldType)
            {
                ...
                {
                    if (value is string s) return ByteString.FromBase64(s);
                    ...
                    if (value is string s)
                    {
                        ...
                        if (enumValueDescriptor == null)
                            throw new InvalidOperationException(
                                $"Invalid enum value '{s}' for enum type {descriptor.EnumType.Name}.");
        ...
        public static void RecursiveSetValue(IMessage currentValue, List<FieldDescriptor> pathDescriptors,
            object values)
        {
        ...
        {
            return m.File.Package == "google.protobuf" && m.File.Name == "google/protobuf/wrappers.proto";
        }
...
#pragma warning disable CS0618 // Type or member is obsolete
            return methodDescriptor.CustomOptions.TryGetMessage(HttpRuleFieldId, out httpRule);
            ...
            if (!string.IsNullOrEmpty(body))
            {
                if (!string.Equals(body, "*", StringComparison.Ordinal))
                {
                    ...
                    if (leafDescriptor.IsRepeated)
                    {
                        ...
                        // type to get the repeating colle.csion.
