...
namespace GRpc.Shared.Server
{
    ...
    internal sealed class
        ClientStreamingServerMethodInvoker<TService, TRequest, TResponse> : ServerMethodInvokerBase<TService, TRequest,
            TResponse>
        ...
        where TService : class
    {
        ...
        private async Task<TResponse> ResolvedInterceptorInvoker(IAsyncStreamReader<TRequest> requestStream,
            ServerCallContext resolvedContext)
        {
            ...
            {
                if (serviceHandle.Instance != null) await ServiceActivator.ReleaseAsync(serviceHandle);
            }
        }
        ...
        /// <returns>
        ///     A <see cref="Task{TResponse}" /> that represents the asynchronous method. The <see cref="Task{TResponse}.Result" />
        ///     property returns the <typeparamref name="TResponse" /> message.
        /// </returns>
        public async Task<TResponse> Invoke(HttpContext httpContext, ServerCallContext serverCallContext,
