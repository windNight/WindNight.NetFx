        ...
        where TResponse : class
    {
            ...
            DuplexStreamingServerMethod<TRequest, TResponse> innerInvoker)
        {
                ...
                DuplexStreamingServerMethod<TRequest, TResponse> next)
            {
                return async (requestStream, responseStream, context) =>
                {
                    ...
                    try
                    {
                        await interceptorHandle.Instance.DuplexStreamingServerHandler(requestStream, responseStream,
                            context, next);
                    }
                    finally
                    {
            ...
            ServerStreamingServerMethod<TRequest, TResponse> innerInvoker)
        {
                ...
                ServerStreamingServerMethod<TRequest, TResponse> next)
            {
                return async (request, responseStream, context) =>
                {
                    ...
                    try
                    {
                        await interceptorHandle.Instance.ServerStreamingServerHandler(request, responseStream, context,
                            next);
                    }
                    finally
                    {
            ...
            UnaryServerMethod<TRequest, TResponse> innerInvoker)
        {
            ...
            static UnaryServerMethod<TRequest, TResponse> BuildInvoker(
                InterceptorActivatorHandle interceptorActivatorHandle, UnaryServerMethod<TRequest, TResponse> next)
            {
        ...
        {
            var interceptorHandle =
                interceptorActivator.Create(serviceProvider, interceptorActivatorHandle.Registration);
        ...
        private class InterceptorActivatorHandle
        {
            ...
            public IGrpcInterceptorActivator GetActivator(IServiceProvider serviceProvider)
            {
                ...
                if (_interceptorActivator == null)
                {
                    ...
                    _interceptorActivator =
                        (IGrpcInterceptorActivator) serviceProvider.GetRequiredService(activatorType);
                }
