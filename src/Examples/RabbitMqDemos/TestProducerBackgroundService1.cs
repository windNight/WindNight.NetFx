...
public class TestProducerBackgroundService1 : BaseProducerBackgroundService
{
    public TestProducerBackgroundService1(IRabbitMqProducerFactory producerFactory,
        IRabbitMqProducerSettings producerSettings)
        : base(producerFactory, producerSettings)
    {
    }

    protected override string ProducerName => "TestProducerSvr1";
    ...
    protected override void DoBackgroundWork(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
            try
            {
                var message = $"{ProducerName}:sendtestmsg:{HardInfo.Now:yyyy-MM-dd HH:mm:ss}";
                var routingKey = $"engrid.test.{ProducerName.ToLower()}.{HardInfo.Now:yyyMMdd}";
