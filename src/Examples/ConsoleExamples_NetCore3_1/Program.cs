...
namespace ConsoleExamples_NetCore3_1
{
    class Program
    {
        ...
        [Alias("Test3")]
        static void TestLogA(string testSting = "ts1", int testInt = 1)
        {
            TestLog();
        }

        [Alias("Test3")]
        static void TestLogB()//(string testSting = "ts2", int testInt = 2)
        {
            TestLog();
        }

        [Alias("Test3")]
        static void TestLogC(string testSting = "ts3")//, int testInt = 3)
        {
            TestLog();
        }
        ...
        {
            var actionName = GetPreActionInfo();
            Console.WriteLine($"Current ActionInfo is {actionName}");

        }

        static string GetPreActionInfo(int skipFrames = 2)
        {
            var st = new StackTrace();
            //StackTrace st = new StackTrace(new StackFrame(true));
            var sf = st.GetFrame(skipFrames);//(skipFrames);

            var frame = new StackFrame(skipFrames);       //偏移一个函数位,也即是获取当前函数的前一个调用函数
            var method = frame.GetMethod();      //取得调用函数

            if (method != null)
            {
                var parameters = string.Join(",", method.GetParameters().Select(m => $"{m.ParameterType.Name} {m.Name}:{m.DefaultValue}"));
                var actionInfo = $"{(method.DeclaringType?.FullName ?? "")}.{method.Name}({parameters})";
        ...
        static void ConfigTest(string[] args)
        {
            var host = Host.CreateDefaultBuilder(args)
                              ...
                              .ConfigureServices((hostContext, services) =>
                              {
                                  ...
                                  ConfigItems.ConfigCenterStart(logService: new DefaultLogService());
